...
const u8 g_TilePattern[29 * 8] =
{
	...
	0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x03, 0x07, // 21: T_DOT_TL (gema sup-izq)
	0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xC0, 0xE0, // 22: T_DOT_TR (gema sup-der)
	0x07, 0x03, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, // 23: T_DOT_BL (gema inf-izq)
	0xE0, 0xC0, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, // 24: T_DOT_BR (gema inf-der)
...
// Colores: paredes con textura SCANLINE (filas pares bg=2, impares bg=12) y
// contorno neon fg=3; esquinas fg=8; gema oro/ambar; pellet indice 5 ciclado
const u8 g_TileColor[29 * 8] =
{
	0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, // 0: T_PATH
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 1: T_WALL+0 (interior)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 2: T_WALL+1 (N)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 3: T_WALL+2 (E)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 4: T_WALL+3 (NE)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 5: T_WALL+4 (S)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 6: T_WALL+5 (NS)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 7: T_WALL+6 (ES)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 8: T_WALL+7 (NES)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 9: T_WALL+8 (W)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 10: T_WALL+9 (NW)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 11: T_WALL+10 (EW)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 12: T_WALL+11 (NEW)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 13: T_WALL+12 (SW)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 14: T_WALL+13 (NSW)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 15: T_WALL+14 (ESW)
	0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, 0x32, 0x3C, // 16: T_WALL+15 (NESW)
	0x82, 0x8C, 0x82, 0x8C, 0x82, 0x8C, 0x82, 0x8C, // 17: T_CNW
	0x82, 0x8C, 0x82, 0x8C, 0x82, 0x8C, 0x82, 0x8C, // 18: T_CNE
	0x82, 0x8C, 0x82, 0x8C, 0x82, 0x8C, 0x82, 0x8C, // 19: T_CSW
	0x82, 0x8C, 0x82, 0x8C, 0x82, 0x8C, 0x82, 0x8C, // 20: T_CSE
	0x11, 0x11, 0x11, 0x11, 0x11, 0x61, 0x61, 0x61, // 21: T_DOT_TL (gema sup-izq)
	0x11, 0x11, 0x11, 0x11, 0x11, 0x61, 0x61, 0x61, // 22: T_DOT_TR (gema sup-der)
	0xA1, 0xA1, 0xA1, 0x11, 0x11, 0x11, 0x11, 0x11, // 23: T_DOT_BL (gema inf-izq)
	0xA1, 0xA1, 0xA1, 0x11, 0x11, 0x11, 0x11, 0x11, // 24: T_DOT_BR (gema inf-der)
...
// Fijas de juego: 6 oro brillante (gema sup), 10 ambar oscuro (gema inf),
// 5 pellet (CICLADA por puerto, cero VRAM).
// Temadas por nivel (neon sobre oscuro): 2 relleno pared, 12 relleno alterno
// (scanline), 3 contorno neon, 8 acento esquinas. Solo el 14 queda de reserva.
const u16 g_BasePal[15] =
{
	RGB16(0, 0, 0), // 1  negro (fondo)
	RGB16(0, 0, 1), // 2  relleno pared (tema 0: cian neon)
	RGB16(2, 7, 7), // 3  contorno pared neon (tema 0)
	RGB16(1, 1, 7), // 4  azul oscuro = frightened
	RGB16(7, 4, 1), // 5  pellet (arranca brillante)
	RGB16(6, 4, 0), // 6  oro brillante (gema, mitad superior)
	RGB16(2, 6, 7), // 7  cian (Inky)
	RGB16(7, 7, 7), // 8  acento esquinas (tema 0)
	RGB16(7, 3, 3), // 9  rojo claro (Blinky)
	RGB16(5, 2, 0), // 10 ambar oscuro (gema, mitad inferior)
	RGB16(6, 6, 4), // 11 amarillo (pac)
	RGB16(0, 1, 2), // 12 relleno alterno scanline (tema 0)
	RGB16(6, 2, 5), // 13 magenta/rosa (Pinky)
	RGB16(5, 5, 5), // 14 default MSX2 (reserva)
	RGB16(7, 7, 7), // 15 blanco (HUD/letras/flash)
};

// Temas neon (nivel % 4): 0 cian / 1 verde matrix / 2 magenta synthwave / 3 ambar
const u16 g_ThemeFill[4]    = { RGB16(0, 0, 1), RGB16(0, 1, 0), RGB16(1, 0, 1), RGB16(1, 0, 0) };
const u16 g_ThemeFillAlt[4] = { RGB16(0, 1, 2), RGB16(0, 2, 1), RGB16(2, 0, 2), RGB16(2, 1, 0) };
const u16 g_ThemeEdge[4]    = { RGB16(2, 7, 7), RGB16(2, 7, 2), RGB16(7, 2, 7), RGB16(7, 5, 1) };
const u16 g_ThemeAccent[4]  = { RGB16(7, 7, 7), RGB16(6, 7, 4), RGB16(7, 5, 7), RGB16(7, 7, 4) };
...
#define SH_COOL       164  // fantasma con gafas de sol (el cian): COOL_A=164, COOL_B=168
                           // mapa de shapes: 43/64 usados
...
// Fantasma con GAFAS DE SOL para el cian (tools/genpac.py ghost_cool): visor
// transparente ancho sobre los ojos + patillas de 1 px hasta el borde del
// cuerpo. En frightened pierde las gafas (usa los SH_FRIGHT compartidos).
const u8 g_CoolPattern[2 * 4 * 8] =
{
	0x03, 0x0F, 0x1F, 0x3F, 0x0F, 0x60, 0x60, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xE7, 0xC3, 0x81, 0xC0, 0xF0, 0xF8, 0xFC, 0xF0, 0x06, 0x06, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x39, 0x0C, 0x02, // COOL_A
	0x03, 0x0F, 0x1F, 0x3F, 0x0F, 0x60, 0x60, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x9C, 0x30, 0x40, 0xC0, 0xF0, 0xF8, 0xFC, 0xF0, 0x06, 0x06, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xE7, 0xC3, 0x81, // COOL_B
};
...
void DrawGhosts()
{
	...
	for (u8 i = 0; i < NUM_GHOSTS; ++i)
	{
		...
		// El cian lleva gafas de sol en estado normal; asustado las pierde
		u8 base;
		if (g_GhState[i] == GST_FRIGHT)
			base = SH_FRIGHT;
		else
			base = (i == GH_CYAN) ? SH_COOL : SH_GHOST;
...
void DrawParade()
{
	...
	for (u8 i = 0; i < NUM_GHOSTS; ++i)
	{
		u8 gbase = (i == GH_CYAN) ? SH_COOL : SH_GHOST;   // el cian con gafas
		VDP_SetSpritePattern(SPRT_GHOST + i, alt ? (gbase + 4) : gbase);
...
void EnterState(u8 s)
{
	...
	case ST_TITLE:
		...
		VDP_SetHorizontalOffset(0);
		ApplyTheme(0);
		// CyclePellet no corre en TITLE: dejar el pellet en su fase brillante
		VDP_SetPaletteEntry(5, PEL_BRIGHT);
...
void main()
{
	...
	VDP_LoadSpritePattern(g_CoolPattern, SH_COOL, 2 * 4);
